...
export default function PatientForm({ onSubmit, isLoading, initialData = {} }) {
  ...
  // Optional UX: focus MRN input after clearing
  const searchMrnRef = useRef(null);
  ...
  // Address must include "Singapore" + 6-digit postal code (aligns with your Mongoose schema)
  const validateAddress = (addr) => {
    const a = (addr ?? "").trim();
    if (!a) {
      return 'Address is required. Example: "10 Kent Ridge Crescent, Singapore 119260"';
    }
  ...
  const hasErrors =
    !!mrnWarning ||
    !!dobWarning ||
    !!emailWarning || // email optional; only errors if non-empty & invalid
    !!phoneWarning ||
    !!genderWarning ||
    !!heightWarning ||
    !!weightWarning ||
  ...
  const handleClearForm = () => {
    ...
    // optional UX: focus MRN input after clearing
    setTimeout(() => searchMrnRef.current?.focus(), 0);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Reset form to empty after a successful save (and focus MRN)
      handleClearForm();
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    className={`mt-1 ${invalidClass(hasSearched && addressWarning)}`}
                    placeholder="10 Kent Ridge Crescent, Singapore 119260"
                    disabled={!hasSearched}
                    required
                    aria-invalid={!!(hasSearched && addressWarning)}
                    aria-describedby="address-help"
                    onBlur={() => handleChange('address', (formData.address || '').trim())}
                  />
                  {hasSearched && addressWarning && (
                    <p id="address-help" className="text-xs text-red-600 mt-1">⚠️ {addressWarning}</p>
                  )}
                </div>

                <div>
                  <Label htmlFor="email" className="text-sm font-medium text-neutral-700">Email (optional)</Label>
                  <Input
                    id="email"
                    type="email"
