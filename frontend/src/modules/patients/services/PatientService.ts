...
export async function findPatientByExactMrn(mrn: string): Promise<Patient | null> {
  ...
  return (
    items.find(
      (p) => (p.medical_record_number || "").trim().toLowerCase() === trimmed.toLowerCase()
    ) || null
  );
}
